...
(function(global,doc,factoryFn){
    ...
    function swipeDirection(x1, x2, y1, y2) {
        return Math.abs(x1 - x2) >= Math.abs(y1 - y2) ? (x1 - x2 > 0 ? 'Left' : 'Right') : (y1 - y2 > 2 ? 'Up' : 'Down');
    }
    ...
    function eventListener(DOM){
        ...
        function touchmove(e){
            ...
            //Math.abs(x1-x2)>2 && now1 - lastTouchTime > 600|| Math.abs(y1-y2)>2
            var now1 = new Date();
            if(Math.abs(x1-x2)>2 && now1 - lastTouchTime > 550){
                //断定此次事件为移动手势
